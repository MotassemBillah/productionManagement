...
<div class="row">
    <div class="col-md-6 col-md-offset-2">
        ...
        <div class="mb_5 clearfix">
            ...
            <input type="text" class="col-md-7" id="invoice_no" name="invoice_no" value="<?php echo Purchase::get_production_invoice(); ?>" readonly required>
        </div>
        <div class="mb_5 clearfix">
    ...
    <table class="table table-bordered tbl_thin" id="check">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <input type="number" step="any" id="per_price_{{ $product->id }}" name="per_qty_price[{{ $product->id }}]" min="0" value="{{ $product->avgPurchasePrice($product->id) }}" class="form-control qty" data-info="{{ $product->id }}" readonly>
                </td>
                <td>
...
//        });
        ...
        $(document).on("submit", "#frm_purchase", function (e) {
            var _form = $(this);
            var _url = "{{ URL::to ('production') }}";
            $.post(_url, _form.serialize(), function (res) {
                if (res.success === true) {
                    _form[0].reset();
                    redirectTo('{{ url('production') }}')
