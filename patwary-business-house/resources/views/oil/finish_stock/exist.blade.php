...
<div class="table-responsive">
    <table class="table table-bordered tbl_thin" id="check">
        ...
        <td>{{ $product->productName($product->finish_product_id) }}<input type="hidden" value="{{ $product->finish_product_id }}" name="product_id[{{ $product->id }}]"></td>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("submit", "#frm_purchase", function (e) {
            var _form = $(this);
            var _url = "{{ URL::to ('finishstocks/opening/update') }}";
            $.post(_url, _form.serialize(), function (res) {
                if (res.success === true) {
                    _form[0].reset();
                    redirectTo('{{ url('finishstocks/opening') }}')
