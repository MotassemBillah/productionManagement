...
<div class="table-responsive">
    <table class="table table-bordered tbl_thin" id="check">
        ...
        <td>{{ $product->categoryName($product->raw_category_id) }}<input type="hidden" value="{{ $product->raw_category_id }}" name="category_id[{{ $product->id }}]"></td>
        <td>{{ $product->productName($product->raw_product_id) }}<input type="hidden" value="{{ $product->raw_product_id }}" name="product_id[{{ $product->id }}]"></td>
        <td>{{ $product->unitName($product->raw_product_id) }}</td>
        <td>
        ...
        <td>
            <input type="number" step="any" id="per_price_{{ $product->id }}" name="per_qty_price[{{ $product->id }}]" min="0" value="{{ $product->per_qty_price }}" class="form-control qty" data-info="{{ $product->id }}">
        </td>
        <td>
            <input type="number" step="any" id="net_price_{{ $product->id }}" name="total_price[{{ $product->id }}]" value="{{ $product->total_price }}" class="form-control" readonly>
        </td>
        </tr>
        @endforeach
        </tbody>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("submit", "#frm_purchase", function (e) {
            var _form = $(this);
            var _url = "{{ URL::to ('rawstocks/opening/update') }}";
            $.post(_url, _form.serialize(), function (res) {
                if (res.success === true) {
                    _form[0].reset();
                    redirectTo('{{ url('rawstocks/opening') }}')
