...
<div id="breadcrumbBar" class="breadcrumb site_nav_links no_bdr_rad clearfix">
    ...
    <div class="col-md-6 col-sm-6 col-xs-6 cxs_10 text-center">
        <h2 class="page-title">Update Sales Challan Order</h2>
    </div>
...
{!! Form::open(['method' => 'PUT', 'url' => 'rice/sales-challan/'.$data->id, 'id'=>'frm_purchase', 'class' => 'form-horizontal']) !!}
{{ csrf_field() }}
...
<div class="row">
    <div class="col-md-6">
        ...
        <div class="mb_5 clearfix">
            ...
            <input type="text" class="col-md-7" id="slip_no" name="slip_no" value="{{ $data->slip_no }}">
        </div>
        <div class="mb_5 clearfix">
        ...
        <div class="mb_5 clearfix">
            ...
            <input type="text" class="col-md-7" id="voucher_no" value="{{ $data->voucher_no }}" name="voucher_no">
        </div>
        <div class="mb_5 clearfix">
    ...
    <table class="table table-bordered tbl_thin" id="check">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <input type="number" step="any" name="bag_quantity[{{ $product->id }}]" value="{{ $product->bag_quantity }}" class="form-control text-center">
                </td>
                <td>
                    <input type="number" id="scale_weight_{{ $product->id }}" data-id="{{ $product->id }}" step="any" name="scale_weight[{{ $product->id }}]" value="{{ $product->scale_weight }}" class="form-control text-center weight">
                </td>
                <td>
                    <input type="number" id="track_weight_{{ $product->id }}" data-id="{{ $product->id }}" step="any" name="track_weight[{{ $product->id }}]" value="{{ $product->track_weight }}" class="form-control text-center weight">
                </td>
                <td>
                    <input type="number" id="net_weight_{{ $product->id }}" data-id="{{ $product->id }}" step="any" name="net_weight[{{ $product->id }}]" value="{{ $product->net_weight }}" class="form-control text-center readonly">
                </td>
            </tr>
            @endforeach
        </tbody>
