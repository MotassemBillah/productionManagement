...
<div class="table-responsive">
    <table class="table table-bordered" id="check">
        ...
        <tbody>
            @foreach ($dataset->items as $data)
            <tr>
                <td>{{ $category->find($data->category_id)->name }}<input type="hidden" value="{{ $data->category_id }}" name="category_id[]"> <input type="hidden" class="form-control" id="bag_weight_{{ $data->after_production_id }}"></td>
                <td>{{ $product->find($data->after_production_id)->name }} <input type="hidden" value="{{ $data->after_production_id }}" name="product_id[]" ></td>
                <td><input type="number" min="0" class="form-control qty" id="quantity_{{ $data->after_production_id }}" value="{{ $data->quantity }}" data-weight ="{{ $product->find($data->after_production_id)->weight }}" data-info="{{ $data->after_production_id }}" name="quantity[]"></td>
                <td><input type="number" class="form-control" id="net_weight_{{ $data->after_production_id }}" value="{{ $data->weight }}" name="net_weight[]" readonly></td> 
            </tr>
            <input type="hidden" name="hid" value="{{ $dataset->id }}">
            @endforeach
        </tbody>
    </table>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("submit", "#frmStock", function (e) {
            var _form = $(this);
            var _url = "{{ URL::to ('flour/finish-osupdate') }}";
