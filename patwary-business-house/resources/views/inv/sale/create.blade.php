    ...
    <table width="100%">
        <tr>
            ...
            <td class="wmd_30 text-right" style="width: 12%;">
                <a class="btn btn-success btn-xs xsw_33" href="{{ url('/inv/sale/cart') }}">{{ trans('words.view_cart') }}</a>
            </td>
        </tr>
    </table>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("click", "#search", function () {
            var _url = "{{ URL::to('inv/sale/search_product') }}";
            var _form = $("#frmSearch");

            if ($("#institute_id").val() == "") {
                $("#ajaxMessage").html("Select Company").addClass('alert-danger').show();
                $("#institute_id").focus();
                return false;
            } else if ($("#business_type_id").val() == "") {
                $("#ajaxMessage").html("Select Business Type").addClass('alert-danger').show();
                $("#business_type_id").focus();
                return false;
            } else if ($("#category_id").val() == "") {
                $("#ajaxMessage").html("Select Category").addClass('alert-danger').show();
                $("#category_id").focus();
                return false;
            } else {
                $("#ajax_content").html("Fetching product information...");
        ...
        $(document).on("click", ".add_to_cart", function () {
            ...
            var _url = "{{ URL::to('inv/sale/add_cart_item') }}";
            var _formdata = {};
