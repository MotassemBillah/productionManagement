...
<div class="col-md-12">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            {!! Form::open(['method' => 'PUT', 'url' => 'production-order/'.$data->id, 'class' => 'form-horizontal']) !!} 
            {{ csrf_field() }}                    
                        ...
                        <table class="table table-bordered table-striped" id="check">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $category->find($stock->category_id)->name }}<input type="hidden" value="{{ $stock->category_id }}" name="category_id[{{ $stock->id }}]" ></td>
                                    <td>{{ $product->find($stock->product_id)->name }} <input type="hidden" value="{{ $stock->product_id }}" name="product_id[{{ $stock->id }}]" ></td>
                                    <td>{{ $stock_quantity }} <input type="hidden"  id="stq_{{ $stock->id }}" name="qty[{{ $stock->id }}]" value="{{ $stock_quantity }}"> </td>
                                    <td class="text-center">{{ $avg_weight }} <input type="hidden"  id="stwpq_{{ $stock->id }}" name="wpq[{{ $stock->id }}]" value="{{ $avg_weight }}"> </td>
