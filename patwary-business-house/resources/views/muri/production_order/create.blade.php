                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="alt_inv" class="form-control" onfocusout="CheckOrder(this.value);" name="order_no" value="<?php echo ProductionOrder::get_production_invoice(); ?>" readonly>
            ...
            <table class="table table-bordered" id="check">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $category->find($data->category_id)->name }}<input type="hidden" value="{{ $data->category_id }}" name="category_id[{{ $data->id }}]" ></td>
                        <td>{{ $product->find($data->id)->name }} <input type="hidden" value="{{ $data->id }}" name="product_id[{{ $data->id }}]" ></td>
                        <td class="text-center">{{ $stock_quantity }} <input type="hidden"  id="stq_{{ $data->id }}" name="qty[{{ $data->id }}]" value="{{ $stock_quantity }}"> </td>
                        <td class="text-center">{{ $avg_weight }} <input type="hidden"  id="stwpq_{{ $data->id }}" name="wpq[{{ $data->id }}]" value="{{ $avg_weight }}"> </td>
...
<script type="text/javascript">
    $(document).ready(function () {
        $(document).on("submit", "#frm_production", function (e) {
            var _form = $(this);
            var _url = "{{ URL::to ('production-order') }}";
            $.post(_url, _form.serialize(), function (res) {
                if (res.success === true) {
                    _form[0].reset();
                    redirectTo('<?= url('production-order') ?>')
