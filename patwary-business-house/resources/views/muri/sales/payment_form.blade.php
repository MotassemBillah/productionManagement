    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <div class="modal-title">
                <strong>Customer :</strong> <?= !empty($model->head_id) ? $subhead->find($model->head_id)->name : ""; ?>
        ...
        <input type="hidden" name="sale_id" value="<?= $model->id; ?>">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <select id="headDebit" class="form-control" name="frm_subhead" required>
                                ...
                                <option value="{{$head->id}}" @if($model->head_id == $head->id) {{ 'selected' }}@endif>{{$head->name}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <select id="subheadDebit" class="form-control" name="frm_particular">
                                ...
                                @foreach($particular->where('subhead_id',$model->head_id)->get() as $pt)
                                <option value="{{$pt->id}}" @if($model->subhead_id == $pt->id) {{ 'selected' }} @endif>{{$pt->name}}</option>
                                @endforeach
                            </select>
                        </div> 
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <select id="headCredit" class="form-control" name="to_subhead" required>
                                <option value="">Select Head</option>
                                @foreach($subhead->all() as $head)
                                <option value="{{$head->id}}" @if($payment->dr_subhead_id == $head->id) {{ 'selected' }}@endif>{{$head->name}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <select id="subheadCredit" class="form-control" name="to_particular">
                                <option value="">Select Sub Head</option>
                                @foreach($particular->where('subhead_id',$payment->dr_subhead_id)->get() as $pt)
                                <option value="{{$pt->id}}" @if($payment->dr_particular_id == $pt->id) {{ 'selected' }} @endif>{{$pt->name}}</option>
                                @endforeach
                            </select>
                        </div> 
                    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on("click", "#savePayment", function (e) {
            ...
            var _url = "{{ URL::to('sales/payment/update') }}";
