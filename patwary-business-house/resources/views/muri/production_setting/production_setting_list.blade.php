...
<?php 
    ...
    $producton_setting=ProductionSetting::get();
 ?>
    <div id="breadcrumbBar" class="breadcrumb site_nav_links no_bdr_rad clearfix">
        ...
        <div class="col-md-6 col-sm-6 col-xs-6 cxs_10 text-center">
            <h2 class="page-title">Production Setting List</h2>
        </div>
        <div class="col-md-3 col-sm-3 col-xs-4 cxs_12 no_pad">
            <ul class="text-right no_mrgn">
                <li><a href="{{ url('/home') }}"><i class="fa fa-fw fa-dashboard"></i> Dashboard</a> <span class="divider">/</span></li><li><span> Production Setting</span></li>
            </ul>                            
        </div>
    </div>
            ...
            <tbody>
                <tr>
                                ...
                                <div class="input-group-btn clearfix">
                                    ...
                                    <div class="col-md-2 col-sm-3 no_pad">
                                        <select id="sortBy" class="form-control" name="sort_by">
                                            ...
                                            <option value="product_no_id" style="text-transform:capitalize">Product Number</option>    
                                            <option value="after_production_id" style="text-transform:capitalize">After Production</option>                              
                                        </select>
                                    </div>
                                    <div class="col-md-2 col-sm-3 no_pad">
                    ...
                    <td class="text-right wmd_30" style="">
                        <a href="{{ url ('/new-production-setting') }}"><button class="btn btn-success btn-sm"><i class="fa fa-plus"></i> New</button></a>
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-striped">
                <tbody>
                    ...
                    <?php $sl=1; ?>
                    @foreach ($producton_setting as $product)
                   <?php  
                        $pro_cat_id=$product->category_id;
                        $pro_id=$product->product_id;
                        $pro_no_id=$product->product_no_id;
                        $after_pro_id=$product->after_production_id;
                        ...
                        $after_pro_name=DB::table('after_production')->where('id', $after_pro_id)->first();
                   ?> 
                    <tr>
                        ...
                        <td>{{ !empty($after_pro_name) ? $after_pro_name->name : '' }} </td>
                        <td>{{ $product->total_weight }}</td>
                        <td>{{ $product->net_weight }}</td>
                        <td>{{ $product->bag_weight }}</td>
                        <td>{{ $product->production_ratio }}  %</td>

                        <td class="text-center">
                            <a class="btn btn-info btn-xs" href="edit-production-settiing/{{ $product->id }}"><i class="fa fa-edit"></i> Edit</a>
                            <a class="btn btn-danger btn-xs" href="production-setting/delete/{{$product->id }}" onClick="return confirm('Are you sure about this action? This cannot be undone!');"><i class="fa fa-trash-o"></i> Delete</a>
                        </td>
                        <?php $sl++;  ?>
                    </tr>
