...
<div class="row">
    ...
    <div class="col-md-4">
        ...
        <div class="mb_5 clearfix">
            ...
            <select class="col-md-7" id="subhead" name="subhead" onchange="weight_measure()">
                @foreach($subheads->where('subhead_id',$data->head_id)->get() as $subhead)
                <option value="{{$subhead->id}}" data-mon="{{ $subhead->mon }}" @if($subhead->id == $data->subhead_id) {{ ' selected' }} @endif>{{$subhead->name}}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div class="col-md-4">
        ...
        <div class="mb_5 clearfix">
            <label class="col-md-5 text-right">Per Bag Weight:</label>
            <input type="number" class="col-md-7" id="wpq" name="wpq" min="0" step="any" onkeyup="weight_measure()" value="{{ $data->weight_per_qty }}" required>
        </div>
        <div class="mb_5 clearfix">
            <label class="col-md-5 text-right">Net Weight:</label>
            <input type="number" class="col-md-7" id="net_weight" name="total_net_weight" value="{{ $data->weight }}" required readonly>
            <small class="text-danger">{{ $errors->first('total_net_weight') }}</small>
        </div>
        <div class="mb_5 clearfix">
            <label class="col-md-5 text-right">Net Mon:</label>
            <input type="number" class="col-md-7" id="mon" name="mon" min="0" value="{{ $data->mon }}" step="any" readonly required>
        </div>
    </div>
</div>
...
<div class="table-responsive">
    <table class="table table-bordered" id="check">
        ...
        <td>
            <input type="number" step="any" id="mon_{{ $product->id }}" name="net_mon[{{ $product->id }}]" value="{{ $product->mon }}" class="form-control" readonly>
        </td>
        <td>
...
<script type="text/javascript">
    ...
    function weight_measure() {
        ...
        var bagWeight = document.getElementById("wpq").value;
        ...
        netWeight = quantity * bagWeight;
        mon = kgToMon(netWeight, mon);
