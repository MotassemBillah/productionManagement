                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered tbl_thin" style="margin: 0;">
                            ...
                            <tr>
                                ...
                                if ($debit->sale_id) :
                                        $items = App\WeightItem::where('weight_id', $debit->sale_id)->get();
                                        ?>
                                        <td style="padding:0;">
                                            <table class="table table-bordered">
                                                @foreach ( $items as $item )
                                                <tr>
                                                    <td>{{ $after_product->find($item->after_production_id)->name }} {{ $item->quantity }} = {{ floor(kgToMon($item->weight)) }} mon {{ kgToSher($item->weight)  }} sher * {{ $item->per_bag_price }} </td>
                                                </tr>
                                                @endforeach
                                            </table>
                                        </td>
                                 <?php  elseif ($debit->raw_sale_id) :
                                    $items = App\WeightItem::where('weight_id', $debit->raw_sale_id)->get();
                                    ?>
                                    <td style="padding:0;">
                                        <table class="table table-bordered">
                                            @foreach ( $items as $item )
