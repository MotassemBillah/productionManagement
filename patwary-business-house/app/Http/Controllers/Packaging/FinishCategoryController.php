...
class FinishCategoryController extends HomeController {
    ...
    public function index() {
        $model = new Category();
        $query = $model->where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', PACKAGING]]);
        $dataset = $query->paginate($this->getSettings()->pagesize);
        return view('packaging.finish_category.index', compact('dataset'));
    }

    public function create() {
        adminMsg();
        return view('packaging.category.create');
    }
    ...
    public function store(Request $r) {
        ...
        try {
            ...
            DB::commit();
            return redirect('packaging/category')->with('success', 'New Record Created Successfully.');
        } catch (Exception $e) {
            DB::rollback();
    ...
    public function edit($id) {
        ...
        return view('packaging.category.edit', compact('data'));
    }
    ...
    public function update(Request $r, $id) {
        ...
        try {
            ...
            DB::commit();
            return redirect('packaging/category')->with('success', 'Category Updated Successfully.');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function search(Request $r) {
        ...
        $query->where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', PACKAGING]]);
        if (!empty($search)) {
            $query->where('name', 'like', '%' . $search . '%');
        }
        $query->limit($item_count);
        $dataset = $query->paginate($item_count);
        return view('packaging.finish_category._list', compact('dataset'));
    }
