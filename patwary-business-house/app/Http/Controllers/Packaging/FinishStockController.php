...
class FinishStockController extends HomeController {
    ...
    public function index() {
        $dataset = Product::where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', PACKAGING]])->get();
        $categories = Category::where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', PACKAGING]])->get();
        $stock = new SalesChallanItem();
        return view('packaging.finish_stock.index', compact('dataset', 'categories', 'stock'));
    }
    ...
    public function details() {
        ...
        $categories = Category::where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', PACKAGING]])->get();
        $stock = new SalesChallanItem();
        return view('packaging.finish_stock.details', compact('dataset', 'categories', 'stock'));
    }
    ...
    public function search(Request $r) {
        ...
        $query = Product::where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', PACKAGING]]);
        if (!empty($category)) {
        ...
        $dataset = $query->get();
        $stock = new SalesChallanItem();
        return view('packaging.finish_stock._list', compact('dataset', 'stock'));
    }
    ...
    public function details_search(Request $r) {
        ...
        $dataset = $query->get();
        $stock = new SalesChallanItem();
        return view('packaging.finish_stock.details_search', compact('dataset', 'stock'));
    }
    ...
    public function opening_stocks() {
        ...
        if ((!empty($data))) {
            $products = $data;
            return view('packaging.finish_stock.exist', compact('products'));
        } else {
            $products = Product::where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', PACKAGING]])->get();
            return view('packaging.finish_stock.create', compact('products'));
        }
    }
