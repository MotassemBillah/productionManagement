...
class RawStockController extends HomeController {
    ...
    public function index() {
        $dataset = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', DALMILL]])->get();
        $categories = Category::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', DALMILL]])->get();
        $stock = new PurchaseChallanItem();
        return view('dal.raw_stock.index', compact('dataset', 'categories', 'stock'));
    }
    ...
    public function details() {
        ...
        $categories = Category::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', DALMILL]])->get();
        $stock = new PurchaseChallanItem();
        return view('dal.raw_stock.details', compact('dataset', 'categories', 'stock'));
    }
    ...
    public function search(Request $r) {
        ...
        $query = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', DALMILL]]);
        if (!empty($category)) {
        ...
        $dataset = $query->get();
        $stock = new PurchaseChallanItem();
        return view('dal.raw_stock._list', compact('dataset', 'stock'));
    }
    ...
    public function details_search(Request $r) {
        ...
        $dataset = $query->get();
        $stock = new PurchaseChallanItem();
        return view('dal.raw_stock.details_search', compact('dataset', 'stock'));
    }
    ...
    public function opening_stocks() {
        ...
        if ((!empty($data))) {
            $products = $data;
            return view('dal.raw_stock.exist', compact('products'));
        } else {
            $products = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', DALMILL]])->get();
            return view('dal.raw_stock.create', compact('products'));
        }
    }
