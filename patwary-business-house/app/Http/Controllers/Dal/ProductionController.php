...
use App\Models\Dal\PurchaseChallan;
use App\Models\Dal\PurchaseChallanItem;

class ProductionController extends HomeController {

    public function index() {
        check_user_access('dal_manage_production');
        $dataset = PurchaseChallan::where('type', 'out')->orderBy('date', 'DESC')->paginate($this->getSettings()->pagesize);
        return view('dal.production.index', compact('dataset'));
    }

    public function productionList() {
        check_user_access('dal_manage_production');
        $categories = Category::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', DALMILL]])->get();
        $dataset = PurchaseChallanItem::where('type', 'out')->orderBy('date', 'DESC')->paginate($this->getSettings()->pagesize);
        return view('dal.production.list', compact('dataset', 'categories'));
    }

    public function create() {
        $products = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', DALMILL]])->orderBy('category_id')->get();
        return view('dal.production.create', compact('products'));
    }
    ...
    public function show($id) {
        ...
        return view('dal.production.details', compact('data'));
    }

    public function edit($id) {
        $data = PurchaseChallan::find($id);
        $products = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', DALMILL]])->orderBy('category_id')->get();
        return view('dal.production.edit', compact('data', 'products'));
    }
    ...
    public function update(Request $r, $id) {
        ...
        try {
            ...
            DB::commit();
            return redirect('dal/production')->with('success', 'Record Updated Successfully.');
        } catch (Exception $e) {
            DB::rollback();
    ...
    public function confirm($id) {
        ...
        try {
            ...
            DB::commit();
            return redirect('dal/production')->with('success', 'Record Processed Successfully');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function reset($id) {
        ...
        try {
            ...
            DB::commit();
            return redirect('dal/production')->with('success', 'PurchaseChallan Order Reset Successfully');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function ledger() {
        ...
        return view('dal.production.ledger', compact('dataset', 'subhead', 'particular', 'category'));
    }
    ...
    public function stocks() {
        ...
        $category = new Category();
        $product = new Product();
        return view('dal.production.stocks', compact('dataset', 'category', 'product', 'stock'));
    }
    ...
    public function stocks_search(Request $r) {
        ...
        $category = new Category();
        $product = new Product();
        return view('dal.production.stocks_search', compact('dataset', 'category', 'product'));
    }
...
// Ajax Functions
    public function search(Request $r) {
        ...
        return view('dal.production._list', compact('dataset'));
    }
    ...
    public function productionListSearch(Request $r) {
        ...
        $dataset = $query->paginate($item_count);
        return view('dal.production.list_search', compact('dataset'));
    }
