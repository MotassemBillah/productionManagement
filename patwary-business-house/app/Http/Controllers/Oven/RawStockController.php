...
class RawStockController extends HomeController {
    ...
    public function index() {
        $dataset = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', OVENFACTORY]])->get();
        $categories = Category::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', OVENFACTORY]])->get();
        $stock = new PurchaseChallanItem();
        return view('oven.raw_stock.index', compact('dataset', 'categories', 'stock'));
    }
    ...
    public function details() {
        ...
        $categories = Category::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', OVENFACTORY]])->get();
        $stock = new PurchaseChallanItem();
        return view('oven.raw_stock.details', compact('dataset', 'categories', 'stock'));
    }
    ...
    public function search(Request $r) {
        ...
        $query = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', OVENFACTORY]]);
        if (!empty($category)) {
        ...
        $dataset = $query->get();
        $stock = new PurchaseChallanItem();
        return view('oven.raw_stock._list', compact('dataset', 'stock'));
    }
    ...
    public function details_search(Request $r) {
        ...
        $dataset = $query->get();
        $stock = new PurchaseChallanItem();
        return view('oven.raw_stock.details_search', compact('dataset', 'stock'));
    }
    ...
    public function opening_stocks() {
        ...
        if ((!empty($data))) {
            $products = $data;
            return view('oven.raw_stock.exist', compact('products'));
        } else {
            $products = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', OVENFACTORY]])->get();
            return view('oven.raw_stock.create', compact('products'));
        }
    }
