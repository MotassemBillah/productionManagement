...
class AfterProductionController extends HomeController {
    ...
    public function index() {
        $model = new Product();
        $query = $model->where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', FLOURMILL]]);
        $dataset = $query->paginate($this->getSettings()->pagesize);
        $categories = Category::where([['is_deleted', 0], ['business_type_id', FLOURMILL]])->get();
        return view('flour.after_production.index', compact('dataset', 'categories'));
    }
    ...
    public function store(Request $r) {
        ...
        try {
            ...
            $model->sale_pflour = $r->sale_pflour;
    ...
    public function search(Request $r) {
        ...
        $query = Product::query();
        $query->where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', FLOURMILL]]);
        if (!empty($sort_by)) {
        ...
        //pr($dataset);
        $categories = Category::where([['is_deleted', 0], ['business_type_id', FLOURMILL]])->get();
        return view('flour.after_production._list', compact('dataset', 'categories'));
    }
