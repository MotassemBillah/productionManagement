...
class RawOpeningStockController extends HomeController {
    ...
    public function index() {
        ...
        if (!empty($data) && count($data) > 0) {
            $dataset = $data;
            return view('flour.raw_opening_stocks.index', compact('dataset', 'category', 'product'));
        } else {
            $dataset = Product::where([['type', 'Raw'], ['is_deleted', 0], ['business_type_id', FLOURMILL]])->get();
            return view('flour.raw_opening_stocks.create', compact('dataset', 'category', 'product'));
        }
    }
    ...
    public function store(Request $r) {
        ...
        try {
            ...
            $resp['url'] = url('flour/raw-opening-stocks');
        } catch (Exception $e) {
            DB::rollback();
            $resp['success'] = false;
