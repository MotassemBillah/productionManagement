...
use App\Models\Inv\PurchaseCart;
use App\Models\Inv\PurchaseItem;
...
class PurchaseController extends HomeController {
    ...
    public function index() {
        ...
        $this->model['breadcrumb_title'] = "Purchase List";
        $this->model['dataset'] = $dataset;
        return view('inv.purchase.index', $this->model);
    }
    ...
    public function items() {
        ...
        $this->model['breadcrumb_title'] = "Purchase Item List";
        $this->model['dataset'] = $dataset;
        return view('inv.purchase.items', $this->model);
    }
    ...
    public function create() {
        ...
        $this->model['breadcrumb_title'] = "Purchase Information";
        $this->model['dataset'] = $dataset;
        $this->model['institute_list'] = $institute_list;
        $this->model['business_types'] = $business_types;
        $this->model['categories'] = $categories;
        return view('inv.purchase.create', $this->model);
    }
    ...
    public function cart() {
        ...
        $_model = new Purchase();
        $dataset = PurchaseCart::where('user_id', Auth::id())->get();
        ...
        return view('inv.purchase.cart_form', $this->model);
    }
    ...
    public function clear_cart() {
        ...
        DB::beginTransaction();
        try {
            $cart_dataset = PurchaseCart::where('user_id', Auth::id())->get();
            //pr($cart_dataset);
    ...
    public function store(Request $r) {
        ...
        try {
            ...
            DB::commit();
            return redirect('inv/purchase')->with('success', 'New record created successfully.');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function edit($id) {
        ...
        $_initituteID = institute_id();
        $model = Purchase::where('_key', $id)->first();
        ...
        return view('inv.purchase.edit', $this->model);
    }
    ...
    public function update(Request $r, $id) {
        ...
        try {
            ...
            $_idArr = $r->key;
            foreach ($_idArr as $_idkey => $_idval) {
                $modelItem = PurchaseItem::find($_idval);
                $modelItem->institute_id = $r->inst_id[$_idval];
                $modelItem->business_type_id = $r->btype_id[$_idval];
            ...
            DB::commit();
            return redirect('inv/purchase')->with('success', 'Record updated successfully.');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function detail($id) {
        ...
        if ($model->is_deleted == 1) {
            return redirect("inv/purchase")->with("danger", "You are trying to access an invalid url.");
        }

        $_modelTrn = new Transaction();
        $particular_net_balance = $_modelTrn->sumPartBalance($model->from_particular_id);

        $this->model['breadcrumb_title'] = "Purchase Information Detail";
        $this->model['model'] = $model;
        $this->model['particular_net_balance'] = $particular_net_balance;
        return view('inv.purchase.detail', $this->model);
    }
    ...
    public function payment($id) {
        ...
        if ($model->process_status == ORDER_PENDING) {
            return redirect("inv/purchase/detail/{$id}")->with("danger", "You are trying to access an invalid url.");
        }
    ...
    public function search(Request $r) {
        ...
        return view('inv.purchase._list', compact('dataset'));
    }
    ...
    public function search_items(Request $r) {
        ...
        $query->orderBy("id", "DESC");
        $dataset = $query->paginate($item_count);
        return view('inv.purchase._item_list', compact('dataset'));
    }
    ...
    public function delete() {
        ...
        try {
            foreach ($_POST['data'] as $id) {
                ...
                $modelTrn = Transaction::where([['institute_id', institute_id()], ['pst_type', 'inv'], ['purchase_id', $data->id]])->get();
                if (!empty($modelTrn)) {
    ...
    public function delete_cart_item(Request $r) {
        ...
        DB::beginTransaction();
        try {
            $data = PurchaseCart::find($_itemID);
            if (!$data->delete()) {
                throw new Exception("Error while deleting record.");
            }
    ...
    public function delete_item(Request $r) {
        ...
        DB::beginTransaction();
        try {
            $data = PurchaseItem::find($_itemID);
            $data->is_deleted = 1;
            //$data->deleted_by = Auth::id();
            //$data->deleted_at = cur_date_time();
            if (!$data->save()) {
    ...
    public function search_product(Request $r) {
        ...
        $dataset = $query->get();
        return view('inv.purchase._product_list', compact('dataset'));
    }

    public function add_to_cart(Request $r) {
        //$this->is_ajax_request(Request $request);
        $resp = array();
        $model = new PurchaseCart();
    ...
    public function process(Request $r) {
        //$this->is_ajax_request(Request $request);
        $resp = array();
        $model = Purchase::where('_key', $r->_id)->first();
        ...
        try {
            foreach ($model->items as $item) {
                ...
                $_avgPriceModel->purchase_id = $model->id;
    ...
    public function reset(Request $r) {
        ...
        try {
            foreach ($_POST['data'] as $id) {
                ...
                $modelTrn = Transaction::where([['institute_id', institute_id()], ['pst_type', 'inv'], ['purchase_id', $_obj->id]])->get();
                if (!empty($modelTrn)) {
                    foreach ($modelTrn as $trn) {
                        if (!$trn->delete()) {
                            throw new Exception("Error while deliting transaction record.");
    ...
    public function create_stock() {
        $dataset = Purchase::where('process_status', ORDER_PROCESSED)->get();
        pr(count($dataset), false);

        $counter = 0;
        foreach ($dataset as $data) {
            $counter++;
            $_oldStocks = Stock::where('purchase_id', $data->id)->get();
            //pr($_oldStocks, false);
            if (!empty($_oldStocks)) {
    ...
    public function update_stock() {
        ...
        foreach ($dataset as $data) {
            $counter++;
            $_stocks = Stock::where('purchase_id', $data->id)->get();
            //pr($_stocks, false);
            if (!empty($_stocks)) {
    ...
    public function update_items() {
        //$dataset = PurchaseItem::whereNull('business_type_id')->get();
        $dataset = PurchaseItem::whereNotNull('business_type_id')->get();
        pr(count($dataset), false);
        exit;
        ...
        foreach ($dataset as $data) {
            $counter++;
            if ($data->business_type_id == 1) {
                $instID = 1;
            }
            if ($data->business_type_id == 2) {
                $instID = 14;
            }
            if ($data->business_type_id == 3) {
                $instID = 15;
            }
            if ($data->business_type_id == 4) {
                $instID = 13;
            }
            if ($data->business_type_id == 5) {
                $instID = 17;
            }
            if ($data->business_type_id == 6) {
                $instID = 16;
            }
            if ($data->business_type_id == 8) {
                $instID = 18;
            }
    ...
    public function update_invoice_no() {
        $dataset = Purchase::orderBy("id", "asc")->get();
        pr(count($dataset), false);
