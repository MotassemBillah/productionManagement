...
use App\Models\Inv\SaleCart;
use App\Models\Inv\SaleItem;
...
class SaleController extends HomeController {
    ...
    public function index() {
        ...
        $this->model['breadcrumb_title'] = "Sale List";
        $this->model['dataset'] = $dataset;
        return view('inv.sale.index', $this->model);
    }
    ...
    public function cart() {
        ...
        $_model = new Sale();
        $dataset = SaleCart::where('user_id', Auth::id())->get();

        $invoice_no = $_model->invoice_number();
        $from_subhead_list = SubHead::where([['institute_id', $_initituteID], ['head_id', PATWARY_HEAD_SALES], ['is_deleted', 0]])->get();
        ...
        return view('inv.sale.cart_form', $this->model);
    }
    ...
    public function clear_cart() {
        ...
        DB::beginTransaction();
        try {
            $cart_dataset = SaleCart::where('user_id', Auth::id())->get();
            //pr($cart_dataset);
    ...
    public function store(Request $r) {
        ...
        try {
            ...
            if (!$model->save()) {
                throw new Exception("Error while saving sale record.");
            }
            ...
            foreach ($_idArr as $_idkey => $_idval) {
                ...
                if (!$modelItem->save()) {
                    throw new Exception("Error while saving sale item record.");
                }
            }
            ...
            DB::commit();
            return redirect('inv/sale')->with('success', 'New record created successfully.');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function edit($id) {
        ...
        $institute_list = Institute::get();
        $from_subhead_list = SubHead::where([['institute_id', $_initituteID], ['head_id', $model->from_head_id], ['is_deleted', 0]])->get();
        ...
        return view('inv.sale.edit', $this->model);
    }
    ...
    public function update(Request $r, $id) {
        ...
        try {
            ...
            $_idArr = $r->key;
            foreach ($_idArr as $_idkey => $_idval) {
                $modelItem = SaleItem::find($_idval);
                $modelItem->institute_id = $r->inst_id[$_idval];
                $modelItem->business_type_id = $r->btype_id[$_idval];
                //$modelItem->sale_id = $model->id;
                $modelItem->invoice_date = date_ymd($r->invoice_date);
                $modelItem->category_id = $r->category_id[$_idval];
                $modelItem->product_id = $r->product_id[$_idval];
                $modelItem->quantity = $r->quantity[$_idval];
                ...
                $modelItem->avg_price = round(($modelItem->rate + $_avg_price), 2);
                if (!$modelItem->save()) {
                    throw new Exception("Error while updating sale item record.");
                }
            }

            DB::commit();
            return redirect('inv/sale')->with('success', 'Record updated successfully.');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function detail($id) {
        ...
        if ($model->is_deleted == 1) {
            return redirect("inv/sale")->with("danger", "You are trying to access an invalid url.");
        }

        $_modelTrn = new Transaction();
        $particular_net_balance = $_modelTrn->sumPartBalance($model->to_particular_id);

        $this->model['breadcrumb_title'] = "Sale Information Detail";
        $this->model['model'] = $model;
        $this->model['particular_net_balance'] = $particular_net_balance;
        return view('inv.sale.detail', $this->model);
    }
    ...
    public function payment($id) {
        ...
        if ($model->process_status == ORDER_PENDING) {
            return redirect("inv/sale/detail/{$id}")->with("danger", "You are trying to access an invalid url.");
        }

        $_modelTrn = new Transaction();
        $payment = $_modelTrn->where([['institute_id', $_initituteID], ['pst_type', 'inv'], ['sale_id', $model->id], ['is_dbl', 1]])->first();
        ...
        if (isset($_POST['submit_form'])) {
            ...
            DB::beginTransaction();
            try {
                $ex_payments = $_modelTrn->where([['institute_id', $_initituteID], ['pst_type', 'inv'], ['sale_id', $model->id], ['is_dbl', 1]])->get();
                ...
                $modelTrn->sale_id = $_POST['sale_id'];
                ...
                $modelTrn->type = "D";
                $modelTrn->voucher_type = RECEIVE_VOUCHER;
    ...
    public function search(Request $r) {
        ...
        return view('inv.sale._list', compact('dataset'));
    }
    ...
    public function delete() {
        ...
        try {
            foreach ($_POST['data'] as $id) {
                ...
                $modelTrn = Transaction::where([['institute_id', institute_id()], ['pst_type', 'inv'], ['sale_id', $data->id]])->get();
                if (!empty($modelTrn)) {
    ...
    public function search_product(Request $r) {
        ...
        $dataset = $query->get();
        return view('inv.sale._product_list', compact('dataset'));
    }
    ...
    public function add_to_cart(Request $r) {
        ...
        try {
            ...
            DB::commit();
            $resp['success'] = true;
            $resp['message'] = 'Record added successfully.';
        } catch (Exception $e) {
            DB::rollback();
            $resp['success'] = false;
            $resp['message'] = $e->getMessage();
    ...
    public function process(Request $r) {
        //$this->is_ajax_request(Request $request);
        $resp = array();
        $model = Sale::where('_key', $r->_id)->first();

        DB::beginTransaction();
        try {
            foreach ($model->items as $item) {
                $_stock = new Stock();
                $_stock->subhead_id = $model->to_subhead_id;
                $_stock->particular_id = $model->to_particular_id;
                $_stock->stock_type = STOCK_SALE;
                $_stock->sale_id = $model->id;
                ...
                $_avgPriceModel->sale_id = $model->id;
                ...
                $_product->savg_price = $_avg_price;
                if (!$_product->save()) {
            ...
            $modelTrn->type = "C";
            $modelTrn->voucher_type = SALES_VOUCHER;
            ...
            foreach ($_POST['data'] as $id) {
                ...
                if (!empty($_obj->items)) {
                    foreach ($_obj->items as $_item) {
                        ...
                        $_product->savg_price = !empty($_lastAvgPrice) ? $_lastAvgPrice : NULL;
                        if (!$_product->save()) {
                            throw new Exception("Error while reseting stocks record.");
                        }
                    }
                ...
                $modelTrn = Transaction::where([['institute_id', institute_id()], ['pst_type', 'inv'], ['sale_id', $_obj->id]])->get();
                if (!empty($modelTrn)) {
                    foreach ($modelTrn as $trn) {
                        if (!$trn->delete()) {
                            throw new Exception("Error while deliting transaction record.");
    ...
    public function create_stock() {
        $dataset = Sale::where('process_status', ORDER_PROCESSED)->get();
        pr(count($dataset), false);

        $counter = 0;
        foreach ($dataset as $data) {
            $counter++;
            $_oldStocks = Stock::where('sale_id', $data->id)->get();
            //pr($_oldStocks, false);
            if (!empty($_oldStocks)) {
            ...
            foreach ($data->items as $item) {
                $_stock = new Stock();
                $_stock->subhead_id = $data->to_subhead_id;
                $_stock->particular_id = $data->to_particular_id;
                $_stock->stock_type = STOCK_SALE;
                $_stock->sale_id = $data->id;
                $_stock->invoice_date = $item->invoice_date;
                $_stock->product_id = $item->product_id;
                $_stock->quantity = $item->quantity;
                if ($_stock->save()) {
    ...
    public function update_stock() {
        ...
        foreach ($dataset as $data) {
            $counter++;
            $_stocks = Stock::where('sale_id', $data->id)->get();
            //pr($_stocks, false);
            if (!empty($_stocks)) {
                foreach ($_stocks as $_stock) {
                    $_stock->subhead_id = !empty($data->to_subhead_id) ? $data->to_subhead_id : NULL;
                    $_stock->particular_id = !empty($data->to_particular_id) ? $data->to_particular_id : NULL;
                    if ($_stock->save()) {
    ...
    public function update_invoice_no() {
        $dataset = Sale::orderBy("id", "asc")->get();
        pr(count($dataset), false);
