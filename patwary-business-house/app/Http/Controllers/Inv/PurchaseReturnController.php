...
use App\Models\Inv\PurchaseReturn;
use App\Models\Inv\PurchaseReturnCart;
use App\Models\Inv\PurchaseReturnItem;
...
class PurchaseReturnController extends HomeController {

    public function index() {
        check_user_access('inv_purchase');
        $model = new PurchaseReturn();
        $query = $model->where('is_deleted', 0)->orderBy("id", "DESC");
        $dataset = $query->paginate($this->getSettings()->pagesize);

        $this->model['breadcrumb_title'] = "Purchase Return List";
        $this->model['dataset'] = $dataset;
        return view('inv.purchase_return.index', $this->model);
    }

    public function create() {
        check_user_access('inv_purchase_create');
        $product = new Product();
        $dataset = $product->getList();

        $this->model['breadcrumb_title'] = "Purchase Return Information";
        $this->model['dataset'] = $dataset;
        $this->model['institute_list'] = Institute::get();
        $this->model['business_types'] = BusinessType::get();
        $this->model['categories'] = Category::get();
        return view('inv.purchase_return.create', $this->model);
    }

    public function cart() {
        check_user_access('inv_purchase_create');
        $_initituteID = institute_id();
        $_model = new PurchaseReturn();
        $dataset = PurchaseReturnCart::where('user_id', Auth::id())->get();
        $from_subhead_list = SubHead::where([['institute_id', $_initituteID], ['head_id', PATWARY_HEAD_PURCHASE], ['is_deleted', 0]])->get();
        $to_subhead_list = SubHead::where([['institute_id', $_initituteID], ['is_deleted', 0]])->whereIn('head_id', [PATWARY_HEAD_DEBTORS, PATWARY_HEAD_CREDITORS])->get();

        $this->model['breadcrumb_title'] = "Purchase Return Information";
        $this->model['dataset'] = $dataset;
        $this->model['invoice_no'] = $_model->invoice_number();
        $this->model['from_subhead_list'] = $from_subhead_list;
        $this->model['to_subhead_list'] = $to_subhead_list;
        return view('inv.purchase_return.cart_form', $this->model);
    }
    ...
    public function clear_cart() {
        ...
        DB::beginTransaction();
        try {
            $cart_dataset = PurchaseReturnCart::where('user_id', Auth::id())->get();
            //pr($cart_dataset);
    ...
    public function store(Request $r) {
        ...
        try {
            ...
            DB::commit();
            return redirect('inv/purchase-return')->with('success', 'New record created successfully.');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function edit($id) {
        check_user_access('inv_purchase_edit');
        $_initituteID = institute_id();
        $model = PurchaseReturn::where('_key', $id)->first();
        ...
        return view('inv.purchase_return.edit', $this->model);
    }
    ...
    public function update(Request $r, $id) {
        ...
        try {
            ...
            $_idArr = $r->key;
            foreach ($_idArr as $_idkey => $_idval) {
                $modelItem = PurchaseReturnItem::find($_idval);
                $modelItem->institute_id = $r->inst_id[$_idval];
                $modelItem->business_type_id = $r->btype_id[$_idval];
                //$modelItem->purchase_return_id = $model->id;
                $modelItem->invoice_date = date_ymd($r->invoice_date);
                $modelItem->category_id = $r->category_id[$_idval];
                $modelItem->product_id = $r->product_id[$_idval];
                $modelItem->quantity = $r->quantity[$_idval];
            ...
            DB::commit();
            return redirect('inv/purchase-return')->with('success', 'Record updated successfully.');
        } catch (Exception $e) {
            DB::rollback();
            return redirect()->back()->with('danger', $e->getMessage());
        }
    ...
    public function detail($id) {
        ...
        if ($model->is_deleted == 1) {
            return redirect("inv/purchase-return")->with("danger", "You are trying to access an invalid url.");
        }

        $_modelTrn = new Transaction();
        $particular_net_balance = $_modelTrn->sumPartBalance($model->from_particular_id);

        $this->model['breadcrumb_title'] = "Purchase Return Detail";
        $this->model['model'] = $model;
        $this->model['particular_net_balance'] = $particular_net_balance;
        return view('inv.purchase_return.detail', $this->model);
    }
    ...
    public function payment($id) {
        ...
        if ($model->process_status == ORDER_PENDING) {
            return redirect("inv/purchase-return/detail/{$id}")->with("danger", "You are trying to access an invalid url.");
        }

        $_modelTrn = new Transaction();
        $payment = $_modelTrn->where([['institute_id', $_initituteID], ['pst_type', 'inv'], ['purchase_return_id', $model->id], ['is_dbl', 1]])->first();
        ...
        if (isset($_POST['submit_form'])) {
            ...
            DB::beginTransaction();
            try {
                $ex_payments = $_modelTrn->where([['institute_id', $_initituteID], ['pst_type', 'inv'], ['purchase_return_id', $model->id], ['is_dbl', 1]])->get();
                ...
                $modelTrn->purchase_return_id = $_POST['purchase_return_id'];
        ...
        //$subhead_list = SubHead::where([['is_deleted', 0], ['institute_id', $_initituteID]])->whereIn('head_id', [PATWARY_HEAD_DEBTORS, PATWARY_HEAD_CREDITORS])->get();
        $particular_list = Particular::where([['is_deleted', 0], ['institute_id', $_initituteID]])->get();

        $this->model['breadcrumb_title'] = "Purchase Return Payment";
        $this->model['model'] = $model;
        $this->model['payment'] = $payment;
        $this->model['subhead_list'] = $subhead_list;
        $this->model['particular_list'] = $particular_list;
        return view('inv.purchase_return.payment', $this->model);
    }
    ...
    public function search(Request $r) {
        ...
        if (!empty($search)) {
            ...
            $query->whereIn('from_particular_id', $partyID);
        }
        if (!empty($_invoice)) {
        ...
        return view('inv.purchase_return._list', compact('dataset'));
    }
    ...
    public function delete() {
        ...
        try {
            foreach ($_POST['data'] as $id) {
                ...
                $modelTrn = Transaction::where([['institute_id', institute_id()], ['pst_type', 'inv'], ['purchase_return_id', $data->id]])->get();
                if (!empty($modelTrn)) {
    ...
    public function delete_cart_item(Request $r) {
        ...
        DB::beginTransaction();
        try {
            $data = PurchaseReturnCart::find($_itemID);
            if (!$data->delete()) {
                throw new Exception("Error while deleting record.");
            }
    ...
    public function delete_item(Request $r) {
        ...
        DB::beginTransaction();
        try {
            $data = PurchaseReturnItem::find($_itemID);
            $data->is_deleted = 1;
            //$data->deleted_by = Auth::id();
            //$data->deleted_at = cur_date_time();
            if (!$data->save()) {
    ...
    public function search_product(Request $r) {
        ...
        $dataset = $query->get();
        return view('inv.purchase_return._product_list', compact('dataset'));
    }

    public function add_to_cart(Request $r) {
        //$this->is_ajax_request(Request $request);
        $resp = array();
        $model = new PurchaseReturnCart();
    ...
    public function process(Request $r) {
        //$this->is_ajax_request(Request $request);
        $resp = array();
        $model = PurchaseReturn::where('_key', $r->_id)->first();
        ...
        try {
            foreach ($model->items as $item) {
                ...
                $_stock->stock_type = STOCK_SALE;
                $_stock->purchase_return_id = $model->id;
                ...
                $_avgPriceModel->purchase_return_id = $model->id;
            ...
            $modelTrn->pst_type = "inv";
            $modelTrn->purchase_return_id = $model->id;
    ...
    public function reset(Request $r) {
        ...
        try {
            foreach ($_POST['data'] as $id) {
                ...
                $modelTrn = Transaction::where([['institute_id', institute_id()], ['pst_type', 'inv'], ['purchase_return_id', $_obj->id]])->get();
                if (!empty($modelTrn)) {
                    foreach ($modelTrn as $trn) {
                        if (!$trn->delete()) {
                            throw new Exception("Error while deliting transaction record.");
    ...
    public function create_stock() {
        $dataset = PurchaseReturn::where('process_status', ORDER_PROCESSED)->get();
        pr(count($dataset), false);

        $counter = 0;
        foreach ($dataset as $data) {
            $counter++;
            $_oldStocks = Stock::where('purchase_return_id', $data->id)->get();
            //pr($_oldStocks, false);
            if (!empty($_oldStocks)) {
            ...
            foreach ($data->items as $item) {
                ...
                $_stock->purchase_return_id = $data->id;
                $_stock->invoice_date = $item->invoice_date;
                $_stock->product_id = $item->product_id;
                $_stock->quantity = $item->quantity;
                if ($_stock->save()) {
    ...
    public function update_stock() {
        ...
        foreach ($dataset as $data) {
            $counter++;
            $_stocks = Stock::where('purchase_return_id', $data->id)->get();
            //pr($_stocks, false);
            if (!empty($_stocks)) {
    ...
    public function update_invoice_no() {
        $dataset = PurchaseReturn::orderBy("id", "asc")->get();
        pr(count($dataset), false);
