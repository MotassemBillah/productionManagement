...
class ProductionStockController extends HomeController {
    ...
    public function index() {
        check_user_access('rice_production_stocks');
        $model = new ProductionStock;
        $drawer = new Drawer;
        $dataset = $model->orderBy('date', 'DESC')->paginate($this->getSettings()->pagesize);
        return view('rice.production_stocks.index', compact('dataset', 'drawer'));
    }

    public function create() {
        check_user_access('rice_production_stocks');
        $production_order = ProductionItem::where([['institute_id', institute_id()], ['process_status', '=', 1], ['stock_status', '=', 0],])->groupBy('production_id')->get();
        return view('rice.production_stocks.create', compact('production_order', 'insList'));
    }
...
//                    }
            ...
            $resp['url'] = url('rice/production-stocks');
        } catch (Exception $e) {
            DB::rollback();
            $resp['success'] = false;
    ...
    public function show($id) {
        ...
        $drawer = new Drawer();
        return view('rice.production_stocks.view', compact('data', 'drawer'));
    }
    ...
    public function edit($id) {
        ...
        return view('rice.production_stocks.edit', compact('data', 'drawer_info'));
    }
    ...
    public function update(Request $r, $id) {
        ...
        try {
            ...
            DB::commit();
            return redirect('rice/production-stocks')->with('success', 'Production Stocks Updated Successfully.');
        } catch (Exception $e) {
            DB::rollback();
    ...
    public function stocks() {
        ...
        $categories = Category::where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', RICEMILL]])->get();
        $stock = new ProductionStockItem();
        return view('rice.production_stocks.stocks', compact('dataset', 'categories', 'stock'));
    }
    ...
    public function details() {
        ...
        return view('rice.production_stocks.details', compact('dataset', 'product', 'drawer'));
    }
    ...
    public function search(Request $r) {
        ...
        $drawer = new Drawer;
        return view('rice.production_stocks._list', compact('dataset', 'drawer'));
    }
    ...
    public function stocks_search(Request $r) {
        ...
        $product = $r->product;
        $search = $r->input('search');
        $query = Product::where([['type', 'Finish'], ['is_deleted', 0], ['business_type_id', RICEMILL]]);
        if (!empty($category)) {
        ...
        $product = new Product();
        $stock = new ProductionStockItem();
        return view('rice.production_stocks.stocks_list', compact('dataset', 'category', 'product', 'stock'));
    }

    public function details_search(Request $r) {
        $query = new ProductionStockItem();
        $item_count = !empty($r->item_count) ? $r->item_count : $this->getSettings()->pagesize;
        $from_date = date_ymd($r->input('from_date'));
        $end_date = !empty($r->input('end_date')) ? date_ymd($r->input('end_date')) : date('Y-m-d');
        $search = $r->input('search');
        $query = is_Admin() ? $model->where('type', 'in') : $model->where([['type', 'in'], ['institute_id', institute_id()]]);
        if (!empty($from_date)) {
            $query->whereBetween('date', [$from_date, $end_date]);
        }
        if (!empty($search)) {
        ...
        return view('rice.production_stocks.details_search', compact('dataset', 'product', 'drawer'));
    }
